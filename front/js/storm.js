...
Storm.formatter = {
	...
	tel : function(tel) {
		if (!Storm.validation.isNull(tel)) {
			return tel.replace(/(^02.{0}|^01.{1}|[0-9]{3})([0-9]+)([0-9]{4})/,
					"$1-$2-$3");
		} else {
			return "";
		}
	...
	fax : function(fax) {
		if (!Storm.validation.isNull(fax)) {
			return fax.replace(/(^02.{0}|^01.{1}|[0-9]{3})([0-9]+)([0-9]{4})/,
					"$1-$2-$3");
		} else {
			return "";
		}
...
Storm.validation = {
	...
	isEmpty : function(obj) {
		return Storm.validation.isUndefined(obj)
				|| Storm.validation.isNull(obj) || obj === '' || obj === 'null'
				|| obj.length === 0;
	},
	...
	isEmptyEditor : function($obj) {
		return $obj.val().indexOf('<img') < 0
				&& jQuery.trim($obj.val().replace(/(<([^>]+)>)/gi, "").replace(
						/&nbsp;/gi, '')) == ""
	}
};
...
Storm.validate = {
	...
	viewExceptionMessage : function(result, formId) {
		...
		jQuery.each(errors, function(idx, error) {
			...
			$target = $form.find('input[name="' + error.name
					+ '"], select[name="' + error.name + '"], textarea[name="'
					+ error.name + '"]');

			if ($target.length === 0) {
				Storm.LayerUtil.alert('모델의 ' + error.name
						+ '의 검증식이 잘못되었거나 해당 데이터가 전송시 누락되었습니다.')
				return false;
			}
...
Storm.CodeUtil = {
	...
	setCodeToOption : function(codeList, $select) {
		var option = '', template = new Storm.Template(
				'<option value="{{dtlCd}}">{{dtlNm}}</option>');
...
/**
 * 파일 업로드 클래스 file, image, excel 함수는 jQuery.Deferred 객체를 반환하므로 done, fail등을
 * 사용가능한다.
 *
 * @type {{file: FileUpload.file, excel: FileUpload.excel, image:
 *       FileUpload.image, fileForm: FileUpload.fileForm, upload:
 *       FileUpload.upload, checkFileSize: FileUpload.checkFileSize}}
 */
Storm.FileUpload = {
	/**
	 * <pre>
	...
	fileForm : function(type) {
		...
		var html = '', accept = '', dfd = jQuery.Deferred();
		switch (type) {
		case 'xls':
		...
		html += '<input type="file" name="uploadFile" id="file_id_commonUpload" class="blind"'
				+ accept + ' />';
		html += '</form>';
		$("body").append(html);
		$("#file_id_commonUpload").on(
				'change',
				function() {
	...
	upload : function(formId) {
		var dfd = jQuery.Deferred(), $form = $('#' + formId), url = $form
				.attr("action")
				|| '/front/common/fileUploadResult.do';
	...
	checkFileSize : function(formId) {
		var $files = jQuery('#' + formId + ' input[type="file"]'), files, file;
			...
			if (files && files.length) {
				for (var j = 0, l = files.length; j < l; j++) {
					...
					if (file.size > Constant.file.maxSize) {
						Storm.LayerUtil.alert('파일 ' + file.name
								+ '의 파일사이즈가 2MB를 초과합니다.');
						return false;
					}
				}
...
Storm.FileDownload = {
	download : function(filePath, fileName) {
		var url = '/front/common/download.do', key, param = {};
...
Storm.DaumEditor = {
	 ...
	 */
	init : function() {
		var url = '/front/daumeditor/sstsEditor.html';
		jQuery('body')
				.append(
	 ...
	 */
	create2 : function(id) {
		var html = Storm.DaumEditor.html, dfd = $.Deferred();
		...
		var config = {
			...
			txIconPath : "/front/daumeditor/images/icon/editor/", /*
																	 * 에디터에 사용되는
																	 * 이미지 디렉터리,
																	 * 필요에 따라
																	 * 수정한다.
																	 */
			txDecoPath : "/front/daumeditor/images/deco/contents/", /*
																	 * 본문에 사용되는
																	 * 이미지 디렉터리,
																	 * 서비스에서 사용할
																	 * 때는 완성된
			...
			canvas : {
				exitEditor : {
				/*
				 * desc:'빠져 나오시려면 shift+b를 누르세요.', hotKey: { shiftKey:true,
				 * keyCode:66 }, nextElement:
		...
		EditorJSLoader.ready(function(Editor) {
			...
			Editor.getCanvas().observeJob(Trex.Ev.__IFRAME_LOAD_COMPLETE,
					function() {
						console.log('finish');
						Editor.modify({
			...
			Editor.getToolbar().observeJob(Trex.Ev.__TOOL_CLICK,
					function(type) {
	 ...
	 *
	 * @param id
	 *            Textarea 아이디 또는 Textarea 아이디 배열
	 */
	setValueToTextarea : function(id) {
		var idArray = [], index, idStr, $textarea, images, allAttachedImages, allAttachedImages, inputs;
		...
		if (!jQuery.isArray(id)) {
			...
			$textarea
					.parents('form')
					.find(
							'input[name^="attachImages["], input[name^="deletedImages["]')
					.remove();
			...
			for (var i = 0; i < images.length; i++) {
				...
				if (images[i].existStage) {
					...
					inputs += '<input type="hidden" name="attachImages['
							+ index + '].orgFileNm" value="'
							+ images[i].data.filename + '" />';
					inputs += '<input type="hidden" name="attachImages['
							+ index + '].tempFileNm" value="'
							+ images[i].data.tempfilename + '" />';
					inputs += '<input type="hidden" name="attachImages['
							+ index + '].fileSize" value="'
							+ images[i].data.filesize + '" />';
					inputs += '<input type="hidden" name="attachImages['
							+ index + '].temp" value="' + images[i].data.temp
							+ '" />';
					index++;
				}
			}
			...
			for (var i = 0; i < allAttachedImages.length; i++) {
				...
				if (allAttachedImages[i].deletedMark) {
					inputs += '<input type="hidden" name="deletedImages['
							+ index + '].tempFileNm" value="'
							+ allAttachedImages[i].data.tempfilename + '" />';
					inputs += '<input type="hidden" name="deletedImages['
							+ index + '].temp" value="'
							+ allAttachedImages[i].data.temp + '" />';
					index++;
				}
			}
	...
	setContent : function(id, content) {
		...
		if (Storm.DaumEditor.initializedCnt > 0) {
			...
			Editor.switchEditor(id);
			console.log(jQuery('#tx_canvas_wysiwyg' + id).contents().find(
					'body').html())
...
Storm.sns = new function() {
	...
	function shareSns(name) {
		...
		window.open(url, name,
				"titlebar=1, resizable=1, scrollbars=yes, width=" + sns.width
						+ ", height=" + sns.height);
	}
 ...
 * 다음 맵
 */
Storm.DaumMap = new function() {
	var mapOption, geocoder;
	...
	/**
	 * divId에 영역에 입력받은 주소의 지도를 출력하고, desc 값을 설명을 표시한다. desc 값은 없으면 표시 하지 않는다.
	 *
	 * @param divId
	 * @param addr
	 * @param desc
	 */
	function setMapToDivByAddress(divId, addr, desc) {
				...
				.addressSearch(
						...
						function(result, status) {
							...
							if (status === daum.maps.services.Status.OK) {
								...
								console.log('다음 지도에서 검색할 수 없는 주소');
							}
						});
	}
	...
	return {
		init : init,
		setMapToDivByAddress : setMapToDivByAddress
	}
};