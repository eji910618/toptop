        ...
        ReOrderUtil = {
            reOrderItemArrSet : function() {
                ...
                $('.tr_order_goods_info').each(function(idx){
                    ...
                    var storeYn = "${orderVO.orderInfoVO.storeYn}";
                    if(storeYn == 'Y') {
                ...
                $('#reOrder_form #basketJSON').val(JSON.stringify(goodsList));
        ...
        <div class="inner">
            <section id="mypage" class="sub shopping">
                ...
                <table class="row_table">
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td>${orderVO.orderInfoVO.ordrNm}</td>
                            <th scope="row">이메일</th>
                            <td>${orderVO.orderInfoVO.ordrEmail}</td>
                        </tr>
                        <tr>
                            <th scope="row">휴대폰번호</th>
                            <td colspan="3">${orderVO.orderInfoVO.ordrMobile}</td>
                        </tr>
                    </tbody>
                </table>
                ...
                <table class="common_table">
                    ...
                    <thead>
                        <tr>
                            ...
                            <!-- //0911 수정 -->
                            <c:choose>
                                <c:when test="${orderVO.orderInfoVO.storeYn eq 'Y'}">
                    ...
                    <tbody>
                        ...
                        <c:set var="totalRow" value="${orderVO.orderGoodsVO.size()}"/>
                        ...
                        <c:forEach var="orderGoodsVO" items="${orderVO.orderGoodsVO}" varStatus="status">
                        <c:set var="tr_class" value=""/>
                        <c:set var="groupFirstYn" value="N"/>
                        <c:set var="goodsPrmtGrpNo" value="${orderGoodsVO.goodsPrmtGrpNo}"/>
                        ...
                        <tr class="tr_order_goods_info" data-goods-no="${orderGoodsVO.goodsNo}" data-ctg-no="${orderGoodsVO.ctgNo}" data-buy-qtt="${orderGoodsVO.ordQtt}"
                                    ...
                                    <a href='<goods:link siteNo="${so.siteNo}" partnerNo="${orderGoodsVO.partnerNo}" url="/front/goods/goodsDetail.do?goodsNo=${orderGoodsVO.goodsNo}" />' class="thumb">
                                    	<c:if test="${orderGoodsVO.goodsSetYn ne 'Y'}">
                                        	<c:set var="imgUrl" value="${fn:replace(orderGoodsVO.goodsDispImgC, '/image/ssts/image/goods', '') }" />
                                       		<img src="<spring:eval expression="@system['goods.cdn.path']" />${imgUrl}?AR=0&RS=100X136" alt="${orderGoodsVO.goodsNm}" />
                                        </c:if>
                            ...
                            <c:choose>
                                <c:when test="${orderVO.orderInfoVO.storeYn eq 'Y'}">
                                    <c:if test="${orderVO.orderInfoVO.ordStatusCd eq '20' && visitScdDt >= toDay}">
                ...
                <table class="row_table">
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td>
                                <div class="pay-chks">
                                    <c:set var="orderPayVO" value="${orderVO.orderPayVO[0]}"/>
                                    <input type="hidden" name="ordNo" id="ordNo" value="${orderVO.orderInfoVO.ordNo}" />
                                    ...
                                    <c:if test="${orderVO.orderPayVO[0].paymentWayCd eq '23'}">
                                        <!-- 카드결제  -->
                                        <input type="hidden" id="realServiceYn" value="${realServiceYn}" readonly="readonly"/>
                                        <input type="hidden" name="pgCd" id="pgCd" value="${orderPayVO.paymentPgCd}" readonly="readonly"/>
                                        <input type="hidden" name="mid" id="mid" value="${mid}" readonly="readonly"/>
                ...
                <c:if test="${orderVO.orderInfoVO.storeYn ne 'Y'}">
                    <h4>배송지정보
                    	<c:if test="${orderVO.orderInfoVO.ordStatusCd eq '20' and dlvrModifyUseYn eq 'Y'}">
                    		<button type="button" class="btn medium bd" onclick="javascript:func_popup_init('.dlvr_modify_popup');" style="float: right; position: relative; top: -8px;">배송지 정보변경</button>
                    	</c:if>
                    </h4>
                    <table class="row_table">
                        <colgroup>
                        ...
                        <tbody>
                            <tr>
                                <th scope="row">수령인</th>
                                <td colspan="3">${orderVO.orderInfoVO.adrsNm}</td>
                            </tr>
                            <tr>
                                <th scope="row">휴대폰번호</th>
                                <td>${orderVO.orderInfoVO.adrsMobile}</td>
                                <th scope="row">연락처</th>
                                <td>
                            ...
                            <tr>
                                <th scope="row">주소</th>
                                <td colspan="3">(${orderVO.orderInfoVO.postNo})&nbsp;${orderVO.orderInfoVO.roadnmAddr}&nbsp;${orderVO.orderInfoVO.dtlAddr}</td>
                            </tr>
                            <tr>
                                <th scope="row">배송메모</th>
                                <td colspan="3">${orderVO.orderInfoVO.dlvrMsg}</td>
                            </tr>
                        </tbody>
                    </table>
                </c:if>
                <div class="btn_wrap">
                    <a href="${_MALL_PATH_PREFIX}/front/order/orderList.do" class="btn bd">목록</a>
                </div>
            </section>
            <form id="reOrder_form">
                <input type="hidden" name="itemArr" id="itemArr" />
                <input type="hidden" name="basketJSON" id="basketJSON" />
            </form>
            <!--- 마이페이지 왼쪽 메뉴 --->
                        ...
                        <div class="my_shopping_wrap">
                            <form id="ctrl_form_cash">
                                ...
                                <table class="tax_table">
                                    ...
                                    <tr>
                                        <th scope="row">주문자명</th>
                                        <td><input type="text" style="width:190px" id="applicantNm" name="applicantNm" value="${orderVO.orderInfoVO.ordrNm}" readonly="readonly"></td>
                                    </tr>
                                    <tr>
                                        <th scope="row">전화번호</th>
                                        <td>
