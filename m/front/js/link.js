jQuery(document).ready(function(){
    ...
    if( sns_add_info_Yn == "Y"){
        window.open(Constant.uriPrefix + "/front/login/snsAddInfoPop.do", "", "width=700,height=600");
    }
...
function move_mypage(){
    if(loginYn){
        location.href = Constant.uriPrefix + Constant.uriPrefix + "/front/member/mypage.do";
    }else{
        Storm.LayerUtil.confirm("로그인이 필요한 서비스입니다. 지금 로그인 하시겠습니까?",
            //확인버튼 클릭, 확인시 로그인페이로 이동하는등의 동작이 필요
            function() {
...
function move_category(no) {
    location.href = Constant.uriPrefix + "/front/search/categorySearch.do?ctgNo="+no;
}

function move_order_detail(no) {
    location.href = Constant.uriPrefix + "/front/order/orderDetail.do?ordNo="+no;
}
...
function move_page(idx){
    if(idx == 'faq'){ // FAQ 목록페이지
        ...
        if(loginYn){
            ...
            Storm.LayerUtil.confirm("로그인이 필요한 서비스입니다. 지금 로그인 하시겠습니까?",function() {location.href= Constant.dlgtMallUrl + "/front/login/viewLogin.do"},'');
        }
        ...
        location.href = Constant.uriPrefix + "/front/company.do?siteInfoCd=04";
    }else if(idx == 'benefitGuide'){ //회원혜택 가이드
        location.href = Constant.uriPrefix + "/front/customer/benefitGuide.do?guideGbCd=01";
    }else if(idx == 'couponGuide'){ //쿠폰/포인트/기프트카드 가이드
    	location.href = Constant.uriPrefix + "/front/customer/couponGuide.do?guideGbCd=02";
    }else if(idx == 'orderGuide'){ //주문/결제 가이드
    	location.href = Constant.uriPrefix + "/front/customer/orderGuide.do?guideGbCd=03";
    }else if(idx == 'deliveryGuide'){ //배송/교환/반품/환불 가이드
    	location.href = Constant.uriPrefix + "/front/customer/deliveryGuide.do?guideGbCd=04";
    }else if(idx == 'asGuide'){ //AS/수선서비스 가이드
    	location.href = Constant.uriPrefix + "/front/customer/asGuide.do?guideGbCd=05";
...
function order_cancel(){
    ...
    Storm.AjaxUtil.getJSONwoMsg(url, param, function(result) {
        ...
        Storm.LayerUtil.confirm("선택한 상품을 "+notiMsg+" 하시겠습니까?", function() {
            url = Constant.uriPrefix + "/front/order/cancelOrder.do";
            Storm.AjaxUtil.getJSONwoMsg(url, param, function(result) {
                if(result.success) {
                    Storm.LayerUtil.alert(notiMsg+" 되었습니다.", "알림").done(function(){
