...
$(document).ready(function(){
    ...
    /* 주문 쿠폰/프로모션 선택 */
    //$(document).on('change', 'select[name="ord_prmt"]', function(){
...
function fnDownloadOrdCoupon(prmtNo){
    var url = Constant.uriPrefix + "/front/order/insertOrdCouponOrderForm.do";
...
function fnDownloadDupltCoupon(prmtNo){
    var url = Constant.uriPrefix + "/front/order/insertDupltCouponOrderForm.do";
    var param = {prmtNo:prmtNo};
...
function setPromotionData() {
    ...
    $('#goods_list div.item').each(function(){
        var $this = $(this);
    ...
    Storm.AjaxUtil.getJSONwoMsg(url, param, function(result) {
        ...
        if(result.success) {
            ...
            $.each(result.resultList, function(idx, obj){
                ...
                //freebieList += '            <em><img src="https://d2gocqzpnajr77.cloudfront.net'+imgPath+'" class="thumb" /></em>';
                freebieList += '            <em><img src="https://imgp.topten10mall.com'+imgPath+'" class="thumb" /></em>';
                //freebieList += '            <em><img src="'+result.resultList[idx].imgPath+'" class="thumb"></em>';
                freebieList += '            <div class="thumb-etc">';
                freebieList += '                <span>'+result.resultList[idx].partnerNm+'</span>';
                freebieList += '                <strong>'+result.resultList[idx].freebieNm+'</strong>';
                if(result.resultList[idx].freebieTypeCd == '1') {
            ...
            html += '<div class="freebie_area" style="margin-top:10px;">';
            html += '    <div class="head">';
            html +=          prmtNm;
            html += '    </div>';
            html += '    <div class="body gift">';
            html += '        <ul>';
            html += '            <li>';
...
function createOrdPromotion(){
    ...
    if(ordPrmtAvailYn) {
        ...
        $.ajax({
            type : "POST",
            url :  Constant.uriPrefix + "/front/order/selectOrdPromotionList.do",
            data : data,
            dataType : "json",
                        ...
                        if(parseFloat(data.resultList[i].prmtApplicableAmt) <= parseFloat(totalAmt) && parseInt(data.resultList[i].prmtApplicableQtt) <= parseInt(goodsCnt)) {
                            if(data.resultList[i].prmtBnfCd2 == '05') { // 무료배송 혜택이면 배송비가 있을 경우에만 쿠폰/프로모션 노출
                                if(dlvrAmt > 0) {
                                    ...
                                    html += '        <label for="ord_prmt'+i+'">&nbsp;</label>';
                                    html += '    </span>';
                                    html += '    <div class="two">' + data.resultList[i].prmtNm + '</div>';
                                    html += '</li>';
                                }
                                ...
                                if(storeYn != 'Y') {
                                    if(data.resultList[i].prmtKindCd == '06' && data.resultList[i].memberCpNo == '0'){
                                        ...
                                        html += '<li>';
                                        html += '   <span class="input_button one">';
                                        html += '        <img style="width:36%;" id="couponDownRadio" src="/m/front/img/common/pay/mobile_coupon_radio.png">';
                                        html += '        <label for="ord_prmt'+i+'">&nbsp;</label>';
                                        ...
                                        html += '        data-partner-no="'+data.resultList[i].partnerNo+'" data-prmt-bnf-value="'+data.resultList[i].prmtBnfValue+'">';
                                        html += '    </span>';
                                        html += '    <div style="color:#999;" class="two">' + data.resultList[i].prmtNm;
                                        html += '    <a onclick="fnDownloadOrdCoupon('+data.resultList[i].prmtNo+')"><img style="width:30%;" id="ordCouponDownImg" src="/m/front/img/common/pay/mobile_coupon_down.png" style="cursor:pointer;margin-bottom:1px;"></a>';
                                        html += '    </div>';
                                        html += '</li>';
                                    } else {
                                        html += '<li>';
                    ...
                    if(parseFloat(data.resultList[i].prmtApplicableAmt) <= parseFloat(totalAmt) && parseInt(data.resultList[i].prmtApplicableQtt) <= parseInt(goodsCnt)) {
                        if(data.resultList[i].prmtBnfCd2 == '05') { // 무료배송 혜택이면 배송비가 있을 경우에만 쿠폰/프로모션 노출
                            if(dlvrAmt > 0) {
                                ...
                                html += '        data-prmt-kind-cd="'+data.resultList[i].prmtKindCd+'" data-member-cp-no="'+data.resultList[i].memberCpNo+'" data-prmt-bnf-cd2="'+data.resultList[i].prmtBnfCd2+'"';
                                html += '        data-partner-no="'+data.resultList[i].partnerNo+'" data-prmt-bnf-value="'+data.resultList[i].prmtBnfValue+'">';
                                html += '        <label for="ord_duplt_prmt'+i+'">&nbsp;</label>';
                                html += '    </span>';
                                html += '    <div class="two">' + data.resultList[i].prmtNm + '</div>';
                                html += '</li>';
                            }
                            ...
                            html += '        <label for="ord_prmt'+i+'">&nbsp;</label>';
                            html += '    </span>';
                            html += '    <div style="color:#999;" class="two"><span style="font-weight:bold;">' + data.resultList[i].prmtNm + '</span>';
                            if(data.resultList[i].prmtBnfCd2 == "01" || data.resultList[i].prmtBnfCd2 == "03" || data.resultList[i].prmtBnfCd2 == "07"){
                                if(data.resultList[i].prmtBnfDcRate != "" && data.resultList[i].prmtBnfDcRate != "0" && data.resultList[i].prmtBnfDcRate != null){
                                    html += '<br/>-' + data.resultList[i].prmtBnfDcRate + '% 할인';
                                    ...
                                    var str = String(data.resultList[i].prmtBnfValue);
                                    html += '<br/>-' + str.replace(/(\d)(?=(?:\d{3})+(?!\d))/g, '$1,') +'원 할인';
                                }
                            }
                            ...
                            html += '    <br><a onclick="fnDownloadDupltCoupon('+data.resultList[i].prmtNo+')"><img style="width:30%;margin-top:5px;" id="ordCouponDownImg" src="/m/front/img/common/pay/mobile_coupon_down_new.png" style="cursor:pointer;margin-bottom:1px;"></a>';
                            html += '    </div>';
                            html += '</li>';
                        } else {
                            html += '<li>';
                            ...
                            html += '        <label for="ord_duplt_prmt'+i+'">&nbsp;</label>';
                            html += '    </span>';
                            html += '    <div class="two"><span style="font-weight:bold;color:black">' + data.resultList[i].prmtNm + '</span>';
                            if(data.resultList[i].prmtBnfCd2 == "01" || data.resultList[i].prmtBnfCd2 == "03" || data.resultList[i].prmtBnfCd2 == "07"){
                                if(data.resultList[i].prmtBnfDcRate != "" && data.resultList[i].prmtBnfDcRate != "0" && data.resultList[i].prmtBnfDcRate != null){
                                    html += '<br/>-' + data.resultList[i].prmtBnfDcRate + '% 할인';
                                    if(data.resultList[i].prmtBnfValue != "" && data.resultList[i].prmtBnfValue != "0" && data.resultList[i].prmtBnfValue != null){
