		...
		Storm.LayerUtil.confirm('초간단결제 가입을 진행하시겠습니까?',
            ...
            function(){ // 가입 취소 버튼 클릭시
            	if(page=='order_form'){
            		...
            		$('#paymentWayCd').val('23').prop('selected',true)
            		$("#paymentWayCd").trigger("click");
...
function wpayPayAuth(){
	...
	Storm.LayerUtil.confirm('초간단결제로 진행하시겠습니까?',
		function(){
			wpayUrl = Constant.dlgtMallUrl + '/front/wpay/wpayPayAuthForm.do';
			window.open(wpayUrl, 'wpayPayAuthForm', popupOption);
		},'','초간단결제'
	);
	
	$("#btn_id_confirm_yes").trigger("click");
}
...
function wpayPayAppl(){
	...
	Storm.LayerUtil.alertCallback('결제를 계속 진행합니다.', function(){
		wpayUrl = Constant.dlgtMallUrl + '/front/wpay/wpayPayApplForm.do';
		window.open(wpayUrl, 'wpayPayApplForm', popupOption);
	});
	
	$("#btn_id_alert_yes").trigger("click");
}
