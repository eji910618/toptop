...
<t:insertDefinition name="defaultLayout">
	...
	<t:putAttribute name="script">
	    ...
	    $(document).ready(function(){
            ...
            Storm.DaumEditor.create('free_insert_area'); // ta_id_content2 를 ID로 가지는 Textarea를 에디터로 설정
	        ...
	        $('.btn_free_insert').on('click', function(e) {
                ...
                Storm.LayerUtil.confirm("수정 하시겠습니까?",
                    function() {

                        if(Storm.validate.isValid('form_id_update')) {

                            Storm.DaumEditor.setValueToTextarea('free_insert_area');  // 에디터에서 폼으로 데이터 세팅

                            var url = '${_FRONT_PATH}/community/updateBbsLett.do';
                            var param = $('#form_id_update').serialize();

                            if (Storm.FileUpload.checkFileSize('form_id_update')) {
                                $('#form_id_update').ajaxSubmit({
                                    url : url,
                                    dataType : 'json',
                                    success : function(result){
                                        Storm.validate.viewExceptionMessage(result, 'form_id_update');
                                        if(result.success){
            ...
            jQuery('#input_id_image').on('change', function(e) {
                if($("#imgOldYn").val()=="Y"){
                   ...
                   $("#imageFile").val("");
                   Storm.LayerUtil.alert("등록된 이미지 파일을 먼저 삭제하여 주세요.");
                }else{
                    var ext = jQuery(this).val().split('.').pop().toLowerCase();
                    if($.inArray(ext, ['gif','png','jpg']) == -1) {
                        Storm.LayerUtil.alert('gif,png,jpg 파일만 업로드 할수 있습니다.');
        ...
        function delOldFileNm(fileNo){
            var url = '${_FRONT_PATH}/community/deleteAtchFile.do';
                            ...
                            <c:if test="${resultModel.data.grpNo eq resultModel.data.lettNo}">
                                ...
                                <tr>
                                    <td class="insert" colspan="2">
                                        <div id="edit" style='height:480px'>
                                ...
                                <tr id ="fileUploadSet1">
                                    ...
                                    <td>
                                        ...
                                        <c:forEach var="fileList" items="${resultModel.data.atchFileArr}" varStatus="status">
                                            ...
                                            <%--<div class="bbs_g_thum" id="imgFile_${fileList.fileNo}"><img src="/image/v.do?type=BBS&path=${fileList.filePath}&id1=${fileList.fileNm}" alt="리스트이미지" width="65px" height="65px"><button type="button" class="reply_del" onclick="delOldImgFileNm('${fileList.fileNo}')" ><img src='../img/product/btn_reply_del.gif' alt=파일삭제'></button></div>--%>
                                            <div class="bbs_g_thum" id="imgFile_${fileList.fileNo}"><img src="/image/bbs/${fileList.filePath}/${fileList.fileNm}" alt="리스트이미지" width="65px" height="65px"><button type="button" class="reply_del" onclick="delOldImgFileNm('${fileList.fileNo}')" ><img src='../img/product/btn_reply_del.gif' alt=파일삭제'></button></div>
                                            </c:if>
                                        </c:forEach>
                                    </td>
                                </tr>
